...
describe('favorite blog', () => {
    ...
    test('when list has only one blog returns that', () => {
        assert.strictEqual(listHelper.favoriteBlog([helper.initialBlogs[0]]), helper.initialBlogs[0])
    })

    test('when list has multiple blogs returns one with most likes', () => {
        assert.strictEqual(listHelper.favoriteBlog(helper.initialBlogs), helper.initialBlogs[2])
    })
